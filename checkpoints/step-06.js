...
// 5a: the same two objects, now items in an array - plus reinforcements.
//     hero and villain carry their POST-BATTLE health, which is why the
//     filter below has a real 0 to deal with.
const roster = [
  hero,
  villain,
...
// 5b-5e: four one-liners. Each takes a FUNCTION - step 3's idea cashing in.
...
console.log(getNames(roster));      // ["Superman","Loki","Storm","Vision","Batman"]
console.log(active(roster).length); // 4  - Loki is on 0 HP after the battle
console.log(totalRescues(roster));  // 169
console.log(strongest(roster).name);// "Vision"

// 5f: JSON round-trip - the exact bridge step 8 crosses.
const text = JSON.stringify(roster);
const back = JSON.parse(text);
console.log(back[0].name); // "Superman"

// Stretch 1: immutable power-up. Spread copies each object, original untouched.
const powerUp = (roster) =>
  roster.map((hero) => ({ ...hero, healthPoints: hero.healthPoints + 20 }));
console.log(powerUp(roster)[0].healthPoints); // 40
console.log(roster[0].healthPoints);          // 20 - unchanged

// Stretch 2: the whole scoreboard in one chained expression.
const scoreboard = active(roster)
  ...
  .join(", ");
console.log(scoreboard); // "Vision (95), Batman (50), Storm (30), Superman (20)"
